...
--*/
{
    ...
    if (MiniYCWatcherData.RecordsAllocated < MiniYCWatcherData.MaxRecordsToAllocate) {

        InterlockedIncrement( &MiniYCWatcherData.RecordsAllocated );

        newBuffer = ExAllocateFromNPagedLookasideList( &MiniYCWatcherData.FreeBufferList );
        ...
        if (newBuffer == NULL) {
            ...
            InterlockedDecrement( &MiniYCWatcherData.RecordsAllocated );
...
--*/
{
    ...
    InterlockedDecrement( &MiniYCWatcherData.RecordsAllocated );
    ExFreeToNPagedLookasideList( &MiniYCWatcherData.FreeBufferList, Buffer );
}
...
--*/
{
    ...
    if (newRecord == NULL) {
        ...
        if (!InterlockedExchange( &MiniYCWatcherData.StaticBufferInUse, TRUE )) {

            newRecord = (PRECORD_LIST)MiniYCWatcherData.OutOfMemoryBuffer;
    ...
    if (newRecord != NULL) {
        ...
        newRecord->LogRecord.SequenceNumber = InterlockedIncrement( &MiniYCWatcherData.LogSequenceNumber );
...
--*/
{
    if (FlagOn(Record->LogRecord.RecordType,RECORD_TYPE_FLAG_STATIC)) {
        ...
        FLT_ASSERT(MiniYCWatcherData.StaticBufferInUse);
        MiniYCWatcherData.StaticBufferInUse = FALSE;
...
BOOLEAN SpyIsWatchedPath(_In_ PUNICODE_STRING path) 
{
	...
	if (!InterlockedExchange(&MiniYCWatcherData.WatchPathInUse, TRUE)) 
	{
		result = FsRtlIsNameInExpression(&MiniYCWatcherData.WatchPath, path, TRUE, NULL);
...
BOOLEAN SpyUpdateWatchedPath(_In_ PUNICODE_STRING path)
{
	...
	if (!InterlockedExchange(&MiniYCWatcherData.WatchPathInUse, TRUE)) 
	{
		if (MiniYCWatcherData.WatchPath.Buffer != NULL) 
		{
			RtlFreeUnicodeString(&MiniYCWatcherData.WatchPath);
		}
		if (path != NULL && path->Buffer != NULL)
		{
			RtlUpcaseUnicodeString(&MiniYCWatcherData.WatchPath, path, TRUE);
		}
...
)
{
	...
	else if (Data->Iopb->MajorFunction == IRP_MJ_CREATE && FlagOn(Data->Iopb->IrpFlags, IRP_CREATE_OPERATION))
	{
		...
		{
			PYC_STREAM_CONTEXT  newContext = NULL;
            DbgPrintEx(DPFLTR_IHVDRIVER_ID, 0, "Allocate Context\n");
            FltAllocateContext(//MiniYCWatcherData.Filter,
                FltObjects->Filter,
                FLT_STREAM_CONTEXT,
                sizeof(YC_STREAM_CONTEXT),
                PagedPool,
                (PFLT_CONTEXT*)&newContext);
            newContext->DeleteOnClose = FALSE;
            newContext->Overwrite = TRUE;
            DbgPrintEx(DPFLTR_IHVDRIVER_ID, 0, "Candidate Overwrite File: %wZ\n", Data->Iopb->TargetFileObject->FileName);
            NTSTATUS status1 = FltSetStreamContext(FltObjects->Instance,
        ...
        else if (FlagOn(Data->Iopb->Parameters.Create.Options, FILE_DELETE_ON_CLOSE)) 
        {
            ...
            FltAllocateContext(//MiniYCWatcherData.Filter,
                ...
                PagedPool,
                (PFLT_CONTEXT*)&newContext);
            newContext->DeleteOnClose = TRUE;
            newContext->Overwrite = FALSE;
            DbgPrintEx(DPFLTR_IHVDRIVER_ID, 0, "Candidate Deleted File: %wZ\n", Data->Iopb->TargetFileObject->FileName);
            NTSTATUS status1 = FltSetStreamContext(FltObjects->Instance,
                Data->Iopb->TargetFileObject,
                FLT_SET_CONTEXT_KEEP_IF_EXISTS,
                newContext,
	...
	{	
        ...
        NTSTATUS status1 = FltQueryInformationFile(Data->Iopb->TargetInstance,
            Data->Iopb->TargetFileObject,
            &fileInfo,
            sizeof(fileInfo),
        ...
        PYC_STREAM_CONTEXT  oldContext = NULL;
        NTSTATUS status2 = FltGetStreamContext(FltObjects->Instance,
            Data->Iopb->TargetFileObject,
            (PFLT_CONTEXT*)&oldContext);

        if (NT_SUCCESS(status1) && oldContext && oldContext->Overwrite)
        {
            DbgPrintEx(DPFLTR_IHVDRIVER_ID, 0, "FIND OVERWRITE: %wZ\n", Data->Iopb->TargetFileObject->FileName);

            FltReleaseContext(oldContext);
            return FILE_SYSTEM_EVENT_OVERWRITE;
        }
    ...
    else if (Data->Iopb->MajorFunction == IRP_MJ_CLEANUP)
    {
        ...
        if (STATUS_FILE_DELETED == status1 && oldContext && oldContext->DeleteOnClose) 
        {
            DbgPrintEx(DPFLTR_IHVDRIVER_ID, 0, "FIND DELETED: %wZ\n", Data->Iopb->TargetFileObject->FileName);
            
            FltReleaseContext(oldContext);
            return FILE_SYSTEM_EVENT_REAL_DELETE;
        }
...
--*/
{
    ...
    KeAcquireSpinLock(&MiniYCWatcherData.OutputBufferLock, &oldIrql);
    InsertTailList(&MiniYCWatcherData.OutputBufferList, &RecordList->List);
    KeReleaseSpinLock(&MiniYCWatcherData.OutputBufferLock, oldIrql);
}
...
--*/
{
    ...
    while (!IsListEmpty( &MiniYCWatcherData.OutputBufferList ) && (OutputBufferLength > 0)) {
        ...
        pList = RemoveHeadList( &MiniYCWatcherData.OutputBufferList );
        ...
        if (OutputBufferLength < pLogRecord->Length) {
            ...
            InsertHeadList( &MiniYCWatcherData.OutputBufferList, pList );
            break;
        }
...
--*/
{
    ...
    while (!IsListEmpty( &MiniYCWatcherData.OutputBufferList )) {
...
--*/
{
    ...
    if (NT_SUCCESS( status )) {
        ...
        MiniYCWatcherData.MaxRecordsToAllocate = *((PLONG)&(pValuePartialInfo->Data));
    }
    ...
    if (NT_SUCCESS( status )) {
        ...
        pValuePartialInfo = (PKEY_VALUE_PARTIAL_INFORMATION) buffer;
        FLT_ASSERT( pValuePartialInfo->Type == REG_DWORD );
        MiniYCWatcherData.NameQueryMethod = *((PLONG)&(pValuePartialInfo->Data));
    }
