...
MINIYCWATCHER_DATA MiniYCWatcherData;
...
--*/
{
    ...
    try {
		...
		MiniYCWatcherData.WatchProcess = 0;
		MiniYCWatcherData.WatchThread = 0;
        MiniYCWatcherData.LogSequenceNumber = 0;
        MiniYCWatcherData.MaxRecordsToAllocate = DEFAULT_MAX_RECORDS_TO_ALLOCATE;
        MiniYCWatcherData.RecordsAllocated = 0;
        MiniYCWatcherData.NameQueryMethod = DEFAULT_NAME_QUERY_METHOD;
		MiniYCWatcherData.ClientPort = NULL;
		MiniYCWatcherData.WatchPathInUse = FALSE;

		RtlInitUnicodeString(&MiniYCWatcherData.WatchPath, NULL);

        MiniYCWatcherData.DriverObject = DriverObject;

        InitializeListHead( &MiniYCWatcherData.OutputBufferList );
        KeInitializeSpinLock( &MiniYCWatcherData.OutputBufferLock );

		ExInitializeNPagedLookasideList( &MiniYCWatcherData.FreeBufferList,
                                         NULL,
                                         NULL,
                                         POOL_NX_ALLOCATION,
                                         RECORD_SIZE,
...
#pragma warning ( pop )
        ...
        RtlInitUnicodeString( &uniString, MINIYCWATCHER_PORT_NAME );
        ...
        status = FltCreateCommunicationPort( MiniYCWatcherData.Filter,
                                             &MiniYCWatcherData.ServerPort,
                                             &oa,
                                             NULL,
                                             SpyConnect,
                                             SpyDisconnect,
        ...
        status = FltStartFiltering( MiniYCWatcherData.Filter );

    } finally {

        if (!NT_SUCCESS( status ) ) {

             if (NULL != MiniYCWatcherData.ServerPort) {
                 FltCloseCommunicationPort( MiniYCWatcherData.ServerPort );
             }

             if (NULL != MiniYCWatcherData.Filter) {
                 FltUnregisterFilter( MiniYCWatcherData.Filter );
             }

             ExDeleteNPagedLookasideList( &MiniYCWatcherData.FreeBufferList );
        }
    }

	DbgPrintEx(DPFLTR_IHVDRIVER_ID, 0, "MiniYCWatcher %i.%i loaded successfully\n", MINIYCWATCHER_MAJ_VERSION, MINIYCWATCHER_MIN_VERSION);
...
--*/
{
    ...
    FLT_ASSERT( MiniYCWatcherData.ClientPort == NULL );
    MiniYCWatcherData.ClientPort = ClientPort;
...
--*/
{
    ...
    FltCloseClientPort( MiniYCWatcherData.Filter, &MiniYCWatcherData.ClientPort );
	MiniYCWatcherData.ClientPort = NULL;
	MiniYCWatcherData.WatchProcess = 0;
	MiniYCWatcherData.WatchThread = 0;
...
break;
            ...
            case GetMiniSpyVersion:
				...
				if ((OutputBufferSize < sizeof(MINIYCWATCHERVER)) ||
					(OutputBuffer == NULL)) {
				...
				try {
					...
					((PMINIYCWATCHERVER)OutputBuffer)->Major = MINIYCWATCHER_MAJ_VERSION;
					((PMINIYCWATCHERVER)OutputBuffer)->Minor = MINIYCWATCHER_MIN_VERSION;

				} except(SpyExceptionFilter(GetExceptionInformation(), TRUE)) {
					return GetExceptionCode();
				}

				*ReturnOutputBufferLength = sizeof(MINIYCWATCHERVER);
			...
			case SetWatchProcess:
				...
				try {
					MiniYCWatcherData.WatchProcess = *((LONGLONG*)((PCOMMAND_MESSAGE)InputBuffer)->Data);
					DbgPrintEx(DPFLTR_IHVDRIVER_ID, 0, "Watching process %li\n", MiniYCWatcherData.WatchProcess);
					status = STATUS_SUCCESS;
				} except(SpyExceptionFilter(GetExceptionInformation(), TRUE)) {
					return GetExceptionCode();
				}
			...
			case SetWatchThread:
				...
				try {
					MiniYCWatcherData.WatchThread = *((LONGLONG*)((PCOMMAND_MESSAGE)InputBuffer)->Data);
					DbgPrintEx(DPFLTR_IHVDRIVER_ID, 0, "Watching thread %li\n", MiniYCWatcherData.WatchThread);
					status = STATUS_SUCCESS;
				} except(SpyExceptionFilter(GetExceptionInformation(), TRUE)) {
					return GetExceptionCode();
				}
...
--*/
{
	...
	if (MiniYCWatcherData.ClientPort == NULL || MiniYCWatcherData.WatchPath.Buffer == NULL)
	{
	...
	CONTINUE_IF_MATCHES(MiniYCWatcherData.WatchProcess, PsGetCurrentProcessId());

	CONTINUE_IF_MATCHES(MiniYCWatcherData.WatchThread, PsGetCurrentThreadId());
	...
	if (Data->Iopb->MajorFunction == IRP_MJ_SET_INFORMATION && Data->Iopb->Parameters.SetFileInformation.FileInformationClass == FileRenameInformation)
	{
		...
		if (info != NULL)
		{
			targetNameStatus = FltGetDestinationFileNameInformation(FltObjects->Instance, FltObjects->FileObject, info->RootDirectory, info->FileName, info->FileNameLength, FLT_FILE_NAME_NORMALIZED | MiniYCWatcherData.NameQueryMethod, &targetNameInfo);
		}
	}

	nameStatus = FltGetFileNameInformation(Data, FLT_FILE_NAME_NORMALIZED | MiniYCWatcherData.NameQueryMethod, &nameInfo);
...
VOID
YCStreamContextCleanupCallback(
    _In_ PYC_STREAM_CONTEXT StreamContext,
